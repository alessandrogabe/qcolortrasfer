...
function paintFinder(bits,modules,ox,oy){for(let y=-1;y<=7;y++)for(let x=-1;x<=7;x++){const gx=ox+x,gy=oy+y;if(gx>=0&&gy>=0&&gx<modules&&gy<modules)bits[gy*modules+gx]=0;}for(let y=0;y<7;y++)for(let x=0;x<7;x++)bits[(oy+y)*modules+ox+x]=finder(x,y)?1:0;}
test('tracked sampler reconstructs a QR-like module matrix without global detection',()=>{
  ...
  paintFinder(expected,modules,0,0);paintFinder(expected,modules,modules-7,0);paintFinder(expected,modules,0,modules-7);
