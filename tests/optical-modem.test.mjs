...
import {
  ...
  MODEM_CODE_BITS,MODEM_CODE_CELLS,MODEM_PALETTE,encodeModemPacket,packetToModemStates,modemStatesToPacket,
  createModemRaster,detectModemMarkers,decodeModemFrame
...
test('clean synthetic modem frame is acquired without QR and decoded end-to-end',async()=>{
  const{packet}=samplePacket(19),raster=createModemRaster(packet,{streamId:0x12345678,symbolId:19}),image=scaleRaster(raster,3);
  const acquisition=detectModemMarkers(image);assert.ok(acquisition);assert.equal(acquisition.markers.length,4);
  const decoded=await decodeModemFrame(image,{forceDetect:true});assert.ok(decoded);assert.equal(decoded.packet.symbolId,19);assert.equal(decoded.packet.chunkSize,MODEM_CHUNK_BYTES);assert.ok(decoded.syncAccuracy>.9);assert.ok(decoded.calibrationSeparation>.1);
});
...
test('standalone modem engines do not invoke QR or ZXing payload paths',async()=>{
  ...
  assert.doesNotMatch(codec,/readBarcodes|QRCode\.create|zxing/i);assert.doesNotMatch(tx,/readBarcodes|QRCode\.create|encodeAuxRepairPacket/);assert.doesNotMatch(rx,/readBarcodes|ZXing|qr-worker/);
