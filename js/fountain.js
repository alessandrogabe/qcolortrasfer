...
export function indicesForSymbol(symbolId, sourceCount) {
  ...
  const random = xorshift32((symbolId ^ (sourceCount * 0x45d9f3b)) >>> 0);
...
export class FountainDecoder {
  ...
  addSymbol(symbolId, data) {
    ...
    for (const [index, solved] of this.solved) {
      if (indices.delete(index)) xorInto(payload, solved);
    }
