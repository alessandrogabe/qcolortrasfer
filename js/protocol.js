...
export const MAGIC = 0x51435431; // QCT1
...
export function encodeOpticalPacket(meta, symbolId, payload) {
  ...
  view.setUint32(8, meta.streamId >>> 0);
  view.setUint32(12, symbolId >>> 0);
  view.setUint32(16, meta.sourceCount >>> 0);
  view.setUint16(20, meta.chunkSize);
  view.setUint16(22, payload.length);
  writeU64(view, 24, meta.fileLength);

  const nameBytes = encoder.encode(meta.fileName || 'file.bin').subarray(0, 27);
  view.setUint8(32, nameBytes.length);
  out.set(nameBytes, 33);
  out.set(hexToBytes(meta.sha256), 60);
  out.set(payload, HEADER_BYTES);
  view.setUint32(HEADER_BYTES + payload.length, crc32(out.subarray(0, HEADER_BYTES + payload.length)));
  return out;
}
...
export function decodeOpticalPacket(bytes) {
  ...
  const payloadLength = view.getUint16(22);
  const end = HEADER_BYTES + payloadLength;
  if (bytes.length < end + 4) throw new Error('Truncated packet');
  const expected = view.getUint32(end);
  const actual = crc32(bytes.subarray(0, end));
  ...
  return {
    ...
    symbolId: view.getUint32(12),
    sourceCount: view.getUint32(16),
    chunkSize: view.getUint16(20),
    fileLength: readU64(view, 24),
    fileName,
    sha256: bytesToHex(bytes.subarray(60, 92)),
