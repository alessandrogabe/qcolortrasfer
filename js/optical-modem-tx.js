...
function install(){
  ...
  function render(item){
    ...
    if(frame)frame.textContent=`OPTICAL MODEM · ${r.rotated?'PORTRAIT':'LANDSCAPE'} · ${layoutName} ${Math.round(layout.cssWidth)}×${Math.round(layout.cssHeight)} CSS px · 192×108 celle · 4 colori · ${MODEM_CHUNK_BYTES} B fountain/frame · RS(255,223)+CRC · ${selectedFps()} fps · ×${scale} raster · ~${theoreticalKiBs().toFixed(1)} KiB/s offerti · ${actual} frame/s · queue ${queue.length}/${LOOKAHEAD} · miss ${misses} · seq ${item.symbolId}`;
    if(gridState)gridState.textContent=`MODEM 192×108 · ${MODEM_STATES} stati · RS(255,223) · ×${scale} · ${layout.desktop?'PC compatto':'mobile'}`;
    if(badge)badge.textContent='OPTICAL MODEM · R/G/B/M · RS';
  }
  ...
  async function prepare(force=false){
    const file=fileInput.files?.[0];if(!file)throw new Error('Seleziona prima un file.');const signature=`${file.name}:${file.size}:${file.lastModified}:${MODEM_CHUNK_BYTES}:rs`;if(!force&&session?.signature===signature)return session;
    setStatus(`OPTICAL MODEM RS: preparo ${file.name}…`);const bytes=new Uint8Array(await file.arrayBuffer()),hash=await sha256Hex(bytes),container=packFileContainerV2(file.name,bytes,hash),streamId=randomStreamId(),encoder=new FountainEncoder(container,MODEM_CHUNK_BYTES,streamId);if(encoder.sourceCount>0xffff)throw new Error('File troppo grande per OPTICAL MODEM.');
    ...
    session={signature,file,bytes,container,encoder,meta,nextSymbolId:0};generation++;queue=[];shown=0;misses=0;if(fileInfo)fileInfo.textContent=`${file.name} · ${formatBytes(bytes.length)} · OPTICAL MODEM RS · K=${encoder.sourceCount} × ${MODEM_CHUNK_BYTES} B · 4 colori`;return session;
  }
  ...
  function enter(){if(active)return;active=true;savedPayload=payload.value;savedFps=fps.value;savedColor=colorMode?.value;savedGrid=grid?.value;let opt=[...payload.options].find(o=>o.value===String(MODEM_CHUNK_BYTES));if(!opt){opt=new Option(`${MODEM_CHUNK_BYTES} B · MODEM RS fisso`,String(MODEM_CHUNK_BYTES));opt.dataset.modem='1';payload.add(opt);}payload.value=String(MODEM_CHUNK_BYTES);payload.disabled=true;if(colorMode)colorMode.disabled=true;if(grid)grid.disabled=true;fps.value=String(DEFAULT_FPS);document.body.dataset.txVariant='optical-modem';session=null;const palette=MODEM_PALETTE.map(c=>`rgb(${c.join(',')})`).join(' · ');setStatus(`OPTICAL MODEM pronto: 192×108, 4 colori R/G/B/M, RS(255,223) byte-interlacciato + CRC + fountain. Layout PC compatto automatico; mobile usa l'area disponibile. Nessun QR/ZXing. ${palette}.`,'ok');}
