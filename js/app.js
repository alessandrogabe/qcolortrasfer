...
import {
  ...
  chooseGridCount, createDualQrRaster, gridDims, COLOR_MODE, VISUAL_STATES
} from './optical.js';

const $ = id => document.getElementById(id);
const FOUNTAIN_CHUNK_BYTES = 512;
const CHANNELS_PER_QR = 2;
const RX_WORKERS = 2;
...
function updateGridLabel() { const { width, height } = txStageBudget(); const side = Math.floor(Math.min(width / state.txCols, height / state.txRows)); $('gridState').textContent = `${state.txSlots} QR · ${state.txCols}×${state.txRows} · ~${side}px`; }
...
function updateTxMeta() {
  ...
  const fps = Number($('fps').value), aggregate = fps * state.txSlots * CHANNELS_PER_QR;
  ...
  const [baseSeq, colorSeq] = state.txLastItem.symbolIds;
  $('txFrame').textContent = `QR V${state.txLastItem.raster.version} ECC ${QR_ECC} · 4 colori / 2 canali · ${state.txSlots} QR · ${fps} fps/QR · ${aggregate} simboli/s teorici · ${actual} generati/s · seq ${baseSeq}+${colorSeq}`;
}

async function makeTxItem(generation) {
  if (!state.encoder || generation !== state.txGeneration) return null;
  const symbolIds = [state.symbolId >>> 0, (state.symbolId + 1) >>> 0]; state.symbolId = (state.symbolId + CHANNELS_PER_QR) >>> 0;
  const baseSymbol = state.encoder.symbol(symbolIds[0]), colorSymbol = state.encoder.symbol(symbolIds[1]);
  const basePacket = encodeOpticalPacket(state.meta, symbolIds[0], baseSymbol.data), colorPacket = encodeOpticalPacket(state.meta, symbolIds[1], colorSymbol.data);
  const raster = await createDualQrRaster(basePacket, colorPacket); if (generation !== state.txGeneration) return null;
  return { symbolIds, degrees: [baseSymbol.indices.length, colorSymbol.indices.length], raster };
}
...
async function rebuildTxGrid(reason = 'layout') {
  ...
  for (let index = 0; index < state.txSlots; index++) {
    ...
    paintTxCell(index, item); state.txSymbolsShown += CHANNELS_PER_QR;
  }
  resizeTxCanvas(); const mode = $('gridMode').value === 'auto' ? 'AUTO' : 'manuale'; log(`TX ${reason}: ${state.txSlots} QR (${state.txCols}x${state.txRows}) · ${mode} · ${VISUAL_STATES} colori · 2 simboli/QR`);
...
async function prepareFile(file) {
  ...
  $('txFileInfo').textContent = `${file.name} · ${formatBytes(bytes.length)} · K=${encoder.sourceCount} × ${encoder.chunkSize} B · LT robust-soliton · SHA-256 ${hash ? 'OK' : 'N/D'}`;
  status('txStatus', 'Pronto. Ogni QR trasporta 2 simboli fountain: uno in luminanza e uno nel colore.', 'ok'); log(`TX preparato: ${file.name}, ${bytes.length} byte, stream ${streamId}, K=${encoder.sourceCount}, ${COLOR_MODE}`);
...
async function txLoop(generation) {
  while (state.transmitting && generation === state.txGeneration) {
    ...
    try { const item = await makeTxItem(generation); if (!item) break; paintTxCell(state.txCellCursor, item); state.txCellCursor = (state.txCellCursor + 1) % state.txSlots; state.txSymbolsShown += CHANNELS_PER_QR; }
...
function startTransmit() { if (!state.encoder || state.transmitting) return; state.transmitting = true; state.txStartedAt = performance.now(); state.txSymbolsShown = 0; const generation = ++state.txGeneration; requestWakeLock(); const aggregate = Number($('fps').value) * state.txSlots * CHANNELS_PER_QR; status('txStatus', `Trasmissione 4 colori attiva: ${state.txSlots} QR · ${$('fps').value} fps/QR · fino a ${aggregate} simboli fountain/s.`, 'ok'); log(`TX start · ${state.txSlots} QR @ ${$('fps').value} fps/QR · dual channel`); void txLoop(generation); }
function stopTransmit() { state.transmitting = false; state.txGeneration++; if (state.encoder) status('txStatus', 'Trasmissione in pausa. I QR colorati visibili restano decodificabili.'); releaseWakeLockIfIdle(); }
...
function renderRxStats() {
  ...
  const colorPct = state.rxColorCandidates ? Math.round(state.rxColorDecoded * 100 / state.rxColorCandidates) : 0; const sep = state.rxColorSeparation ? ` · sep ${state.rxColorSeparation.toFixed(2)}` : '';
  $('rxStats').textContent = `${distinct} simboli distinti · ${dup} duplicati · base ${state.rxBaseDecoded} · colore ${state.rxColorDecoded}/${state.rxColorCandidates} (${colorPct}%)${sep} · ${state.rxCaptured} frame camera · ${state.rxDroppedBusy} saltati · ${state.rxPacketRejected} rifiutati · peeling ${solved}/${total || '—'} · target ~${target || '—'}`;
}
...
function ensureWorkers() {
  ...
  for (let i = 0; i < RX_WORKERS; i++) {
    ...
    worker.onmessage = event => {
      const { id, ready, symbols = [], baseCount = 0, colorCandidates = 0, colorCount = 0, colorSeparation = 0, error } = event.data || {};
      if (id === -1) { if (ready) log(`ZXing worker ${i + 1}/${RX_WORKERS} pronto · multi-QR + 4 colori`); else { state.rxWorkerErrors++; status('rxStatus', `Decoder ZXing/colore non si inizializza: ${error}`, 'error'); log(`Worker init error: ${error}`); } return; }
      state.workerBusy[i] = false; if (error) { state.rxWorkerErrors++; if (state.rxWorkerErrors <= 3) log(`ZXing worker error: ${error}`); }
      state.rxBaseDecoded += baseCount; state.rxColorCandidates += colorCandidates; state.rxColorDecoded += colorCount; if (colorSeparation > 0) state.rxColorSeparation = colorSeparation;
      if (symbols.length) void onDecodedSymbols(symbols); renderRxStats();
    };
...
async function startCamera() {
  ...
  try {
    ...
    const settings = state.track?.getSettings?.() || {}; status('rxStatus', `Camera ${settings.width || video.videoWidth}×${settings.height || video.videoHeight}@${Math.round(settings.frameRate || 0)} · base ZXing + secondo QR cromatico.`, 'ok'); log(`RX camera: ${state.track?.label || 'video'} · ${video.videoWidth}x${video.videoHeight}`); scheduleCapture(state.captureGeneration);
...
function resetReceiver() { state.rxDecoder = null; state.rxMeta = null; state.rxCaptured = 0; state.rxDroppedBusy = 0; state.rxBaseDecoded = 0; state.rxColorCandidates = 0; state.rxColorDecoded = 0; state.rxColorSeparation = 0; state.rxPacketRejected = 0; state.rxWorkerErrors = 0; state.expectedHash = null; state.rxStartedAt = performance.now(); $('rxProgress').value = 0; if (state.downloadUrl) URL.revokeObjectURL(state.downloadUrl); state.downloadUrl = null; const download = $('download'); download.hidden = true; download.removeAttribute('href'); renderRxStats(); status('rxStatus', state.receiving ? 'Ricevitore azzerato. Continua a inquadrare l’intera griglia colorata.' : 'Ricevitore azzerato.'); log('RX reset'); }
...
async function acceptPacket(packet) {
  ...
  const elapsed = Math.max(0.001, (performance.now() - state.rxStartedAt) / 1000); const validRate = (state.rxDecoder.framesNew / elapsed).toFixed(1); renderRxStats(); status('rxStatus', `Ricezione ~${pct}% · ${state.rxDecoder.framesNew}/${target} simboli distinti · ${validRate}/s · colore ${state.rxColorDecoded}/${state.rxColorCandidates}`, 'ok');
...
async function runSelfTest() {
  try {
    const bytes = Uint8Array.from({ length: 1024 }, (_, i) => (i * 37 + 11) & 255); const streamId = 0x12345678; const enc = new FountainEncoder(bytes, FOUNTAIN_CHUNK_BYTES, streamId);
    const meta = { streamId, sourceCount: enc.sourceCount, chunkSize: enc.chunkSize, fileLength: bytes.length, fileName: 'selftest.bin', sha256: 'ab'.repeat(32) };
    const base = encodeOpticalPacket(meta, 0, enc.symbol(0).data), color = encodeOpticalPacket(meta, 1, enc.symbol(1).data);
    if (base.length !== HEADER_BYTES + FOUNTAIN_CHUNK_BYTES + 4) throw new Error('dimensione QCT1 inattesa'); if (decodeOpticalPacket(base).symbolId !== 0 || decodeOpticalPacket(color).symbolId !== 1) throw new Error('QCT1 dual roundtrip');
    const raster = await createDualQrRaster(base, color); if (raster.visualStates !== 4 || raster.coloredModules <= 0) throw new Error('layer colore non attivo');
    status('selfTest', `Autotest: OK · 4 colori · 2 QR sovrapposti · V${raster.version} ECC ${raster.ecc} · ${raster.coloredModules} moduli cromatici · griglia fino a ${MAX_GRID_CODES}.`, 'ok'); log(`Autotest dual-color OK · QR V${raster.version}, ${raster.modules} moduli, ${raster.coloredModules} colorati`);
...
$('capacity').textContent = `${VISUAL_STATES} colori · 2 canali/QR · fino a ${MAX_GRID_CODES} QR · LT robust-soliton`;
